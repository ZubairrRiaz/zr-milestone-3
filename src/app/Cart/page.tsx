...
const CartPage = () => {
    ...
    const calculateSummary = () => {
        ...
        const totalPrice = cartItems.reduce((acc, item) => acc + Number(item.discountPrice), 0);
                ...
                <div className="w-full max-w-4xl bg-white shadow-md rounded-lg p-6">
                    {cartItems.map(item => (
                        <div key={item.id} className="flex flex-col md:flex-row justify-between items-center border-b py-4">
                            <Image src={item.image} alt={item.name} height={300} width={300} className="w-32 h-40 object-cover rounded-md mb-4 md:mb-0" />
                            <div className="flex-1 md:ml-4">
                                <h2 className="text-xl font-semibold text-[#4a4e69]">{item.name}</h2>
                                <p className="text-gray-600">This is a great product that you will love!</p>
                                <p className="text-gray-500">Reviews: {item.reviews}</p>
                                <p className="text-gray-500">PKR: {item.discountPrice}</p>
                            </div>
                            <button
                    ...
                    <button className="w-full mt-6 bg-[#9fc6d2] text-white py-3 rounded-lg font-semibold hover:bg-[#88b0c0] transition duration-300">
                        Proceed to Checkout
                    </button>
                </div>
            </div>
